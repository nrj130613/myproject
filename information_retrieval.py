...
class IRSystem:
    ...
    def compute_tfidf(self):
        ...
        for doc_id, words in self.docs.items():
            for word in words:
                if word in computed_tfidf:
                    computed_tfidf[word].update({doc_id:int()})
        
        for doc_term, doc in self.inv_index.items():
            for id, pos in doc.items():
                tf = len(self.inv_index[doc_term][id])
                tfidf = (1+math.log10(tf)) * idfs[doc_term]
                computed_tfidf[doc_term][id] = tfidf

        return computed_tfidf
        # ------------------------------------------------------------------
    ...
    def get_tfidf(self, word, document):
        ...
        #       document index.
        tfidf = 0.0
        computed_tfidf = self.compute_tfidf()
        tfidf = computed_tfidf[word][document]
        # ------------------------------------------------------------------
        return tfidf
    ...
    def index(self):
        ...
        for id, words in enumerate(self.docs): 
            for i in range(len(words)):
                if words[i] in self.inv_index:
                    self.inv_index[words[i]][id].append(i)
    ...
    def get_posting(self, word):
        ...
        posting = []
        for doc_id, pos in self.inv_index[word].items():
            if word in self.inv_index:
    ...
    def boolean_retrieve(self, query):
        ...
        postings_lists = [self.inv_index.get(term, {}) for term in query]
        doc_ids = set.intersection(*[set(pl.keys()) for pl in postings_lists])
        docs = list(doc_ids)
        return docs
    ...
    def phrase_retrieve(self, query):
        ...
        query_appearing_docs = []
        postings_lists = [self.inv_index.get(term, {}) for term in query]
        doc_ids = set.intersection(*[set(pl.keys()) for pl in postings_lists])
        query_appearing_docs = list(doc_ids)
        docs = []

        for id in query_appearing_docs:
            pos_list = []
            for i in range(len(query)):
                invind = self.inv_index[query[i]][id]
                computed_pos = []
                computed_pos_set = set()
                for pos in invind:
                    new_pos = pos - i 
                    computed_pos.append(new_pos)
                    computed_pos_set = set(computed_pos)
                pos_list.append(computed_pos_set)
            doc = set.intersection(*pos_list)
            if len(doc) > 0:
                docs.append(id)
    ...
    def rank_retrieve(self, query):
        ...
        for id, words in self.docs.items():
            sum_tfidf = float()
            for w in words:
                tfidf = computed_tfidf[w][id]
                tfidf_square = tfidf * tfidf
                sum_tfidf += tfidf_square
            c = math.sqrt(sum_tfidf)
            id_to_c[id] = c
        ...
        for id, words in self.docs.items():
            sum = float()
            for term in query:
                for doc, tfidf in computed_tfidf[term].items():
                    if id == doc:
                        sum += tfidf
            query_score[id] = sum
    

        for id, q_score in query_score.items():
            for d_id, d_score in id_to_c.items():
                if id == d_id:
                    score = q_score/d_score
            scores[id] = score
        # ------------------------------------------------------------------
